...
_MINIMUM_MARGIN = 1.0
_SWITCH_MODEL = json.loads(_find("msato_switch_model.json").read_text(encoding="utf-8"))
_SWITCH_WEIGHTS = {
    str(key): float(value) for key, value in _SWITCH_MODEL["weights"].items()
}
_SWITCH_SUPPORT = Counter({
    str(key): int(value)
    for key, value in _SWITCH_MODEL["descriptor_support"].items()
})
_SWITCH_MINIMUM_SUPPORT = int(_SWITCH_MODEL["minimum_support"])
_SWITCH_MINIMUM_MARGIN = float(_SWITCH_MODEL["minimum_margin"])
...
def _model_pick(obs):
    ...
    if context == 0:
        ...
        support = _SWITCH_SUPPORT
        minimum_support = _SWITCH_MINIMUM_SUPPORT
        minimum_margin = _SWITCH_MINIMUM_MARGIN
    else:
        return None
    options = select.get("option") or []
    ...
    for index, option in enumerate(options):
        score = sum(weights.get(key, 0.0) * value for key, value in _features(obs, option).items())
        scored.append((score, index))
    scored.sort(reverse=True)
    best_score, best_index = scored[0]
    margin = best_score - scored[1][0]
    descriptor = _descriptor(obs, options[best_index])
    if support[descriptor] < minimum_support or margin < minimum_margin:
        return None
    return [best_index]
