...
def heuristic_scores(obs):
    ...
    # Night Stretcher can recover only Basic Energy, not Telepath Energy.  In
    # the exact attack-starvation state below, recovering a Pokemon first can
    # spend the only Stretcher without restoring this turn's attack.  Keep the
    # root play policy unchanged; this guard only resolves the already-open
    # TO_HAND sub-selection.
    stretcher_energy_emergency = (
        context == SelectContext.TO_HAND
        and select.effect is not None
        and select.effect.id == Night_Stretcher
        and active_id in (Kadabra, Alakazam)
        and not active_has_psychic
        and not state.energyAttached
        and hand_counts[Basic_Psychic_Energy] == 0
        and hand_counts[Telepath_Psychic_Energy] == 0
        and discard_counts[Basic_Psychic_Energy] >= 1
    )
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            # Stage-2 Alakazam needs the evolution window more than an
            # immediate attack.  Going first lets Abra evolve before the
            # opponent's second turn, which is especially important against
            # Starmie bench damage and in Alakazam mirrors.
            score = 5 if context == SelectContext.IS_FIRST else 1
        elif o.type == OptionType.NO:
            score = -1 if context == SelectContext.IS_FIRST else 0
            ...
            if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                ...
                if stretcher_energy_emergency:
                    score = 100000 if card.id == Basic_Psychic_Energy else -100000
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if cid == Buddy_Buddy_Poffin:
                    ...
                    elif op_active is not None and count_special_defense_energies(op_active) > 0: score = W["hammer_any"]
                    else: score = -1
        ...
        elif (select.effect is not None
              and select.effect.id == Enhanced_Hammer
              and ((o.type == OptionType.ENERGY and context == SelectContext.DISCARD_ENERGY)
                   or (o.type == OptionType.ENERGY_CARD and context == SelectContext.DISCARD_ENERGY_CARD))):
            score = -1
            pokemon = get_card(obs, o.area, o.index, o.playerIndex)
            if pokemon is not None and o.energyIndex is not None:
                if 0 <= o.energyIndex < len(pokemon.energyCards):
                    energy = pokemon.energyCards[o.energyIndex]
                    if energy.id in (Mist_Energy, Rock_Fighting_Energy):
                        score = 1000
                        if o.area == AreaType.ACTIVE:
                            score += 5000
                        if target_pokemon is not None and pokemon.serial == target_pokemon.serial:
                            score += 10000
...
if not _TEMPLATES:
    _belief_candidates = [
        "/kaggle_simulations/agent/marnie_belief.json",
        "marnie_belief.json",
    ]
    if "__file__" in globals():
        _belief_candidates.append(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "marnie_belief.json")
        )
    for _belief_path in _belief_candidates:
        if not os.path.isfile(_belief_path):
            continue
        try:
            with open(_belief_path, encoding="utf-8") as _f:
                _belief_entries = json.load(_f)
            for _entry in _belief_entries:
                _ids = [int(card_id) for card_id in _entry["deck"]]
                if len(_ids) != 60:
                    continue
                _name = str(_entry["name"])
                _TEMPLATES.append((_name, Counter(_ids), _ids))
        except Exception:
            _TEMPLATES = []
        break
...
_TEMPLATE_VARIANTS = defaultdict(list)
for _name, _signature, _counter, _ids in _TEMPLATE_SIG:
    _parts = _name.rsplit(".", 1)[0].split("__")
    _family = _parts[0]
    _weight = 1
    for _part in _parts[1:]:
        if _part.startswith("w") and _part[1:].isdigit():
            _weight = max(1, int(_part[1:]))
    _TEMPLATE_VARIANTS[_family].append((_counter, _ids, _weight))
_TEMPLATE_SUPPORT = {
    family: sum(weight for _, _, weight in variants)
    for family, variants in _TEMPLATE_VARIANTS.items()
}
MIN_LIVE_BELIEF_SUPPORT = 4
...
def _match_archetype(op_seen):
    """Sample a recent legal deck variant after a core line is visible."""
    op_mons = {cid for cid in op_seen if _card_data.get(cid) and
               _card_data[cid].cardType == CardType.POKEMON}
    if not op_mons or not _TEMPLATE_VARIANTS:
        return None
    # Great Tusk lists also contain the complete Crustle line.  Crustle alone
    # is intentionally left ambiguous; the search gate opens only once Great
    # Tusk itself is visible.
    if 58 in op_mons:
        best_family = "great_tusk"
    else:
        best_family = None
        best_n = 0
        for family, core in _TEMPLATE_CORE.items():
            n = len(core & op_mons)
            if n > best_n:
                best_family, best_n = family, n
    if best_family is None:
        return None
    # Only the Marnie family produced a repeatable gain in the expanded
    # screen (40/40 versus the v29 control's 37/40).  All other families use
    # the exact established v29 fallback until they have independent positive
    # evidence, even when enough replay variants are available.
    if best_family != "marnie":
        return None
    # A one- or two-game family distribution is too brittle to replace the
    # established policy's search belief.  Low-support and unknown families
    # deliberately return None so _sample_hidden follows the original v29
    # synthetic fallback instead.
    if _TEMPLATE_SUPPORT.get(best_family, 0) < MIN_LIVE_BELIEF_SUPPORT:
        return None
    variants = _TEMPLATE_VARIANTS.get(best_family, [])
    if not variants:
        return None
    counter, ids, _ = random.choices(
        variants,
        weights=[weight for _, _, weight in variants],
        k=1,
    )[0]
    return counter, ids
...
def _search_decide(obs, base_order, base_scores):
    ...
    began = False
    # Search rollouts call the same greedy scorer as the live policy.  Preserve
    # the live turn's stateful ability flags so simulated Dudunsparce or
    # Fezandipiti activations cannot suppress a real activation afterwards.
    saved_heuristic_state = (
        pre_turn,
