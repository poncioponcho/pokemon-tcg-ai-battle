...
def _model_pick(obs):
    ...
    if not isinstance(select, dict) or select.get("context") != 0:
        return None
    # This model was trained only on expert games whose complete opponent deck
    # was Grimmsnarl.  Deployment still obeys the visible-information contract:
    # do not activate until a Grim signature card is actually on the board.
    if "Grimmsnarl" not in _visible_archetype(obs).split("+"):
        return None
    options = select.get("option") or []
