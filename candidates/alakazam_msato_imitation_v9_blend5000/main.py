...
_RAW_OBS = None
_BLEND_ALPHA = 5000.0
_STRONGGUARD = _PARENT._BASE
_BASE_MAIN = _STRONGGUARD._BASE
_PARENT_HEURISTIC = _STRONGGUARD.heuristic_scores


def _blended_heuristic(class_observation):
    scores = list(_PARENT_HEURISTIC(class_observation))
    raw = _RAW_OBS
    try:
        select = raw.get("select") if isinstance(raw, dict) else None
        options = (select or {}).get("option") or []
        if not isinstance(select, dict) or select.get("context") != 0:
            return scores
        if len(options) != len(scores) or len(options) < 2:
            return scores
        residuals = [
            sum(_WEIGHTS.get(key, 0.0) * value for key, value in _features(raw, option).items())
            for option in options
        ]
        center = sum(residuals) / len(residuals)
        return [
            score + _BLEND_ALPHA * (residual - center)
            for score, residual in zip(scores, residuals)
        ]
    except Exception:
        return scores


_BASE_MAIN.heuristic_scores = _blended_heuristic
_STRONGGUARD._V24.heuristic_scores = _blended_heuristic
