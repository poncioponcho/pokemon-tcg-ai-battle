...
from .damage_protocol import ManualDamageProtocol
from .target_protocol import ManualTargetProtocol
from .count_protocol import ManualCountProtocol
...
class TargetEvaluator:
    ...
    def __init__(self) -> None:
        self.damage_protocol = ManualDamageProtocol()
        self.target_protocol = ManualTargetProtocol()
        self.count_protocol = ManualCountProtocol()

    def select(self, view: dict) -> list[int]:
        ctx = int(view.get("ctx", -1))
        if ctx == profile.REMOVE_DAMAGE:
            base = profile._remove_damage_source(view)
            return self.damage_protocol.resolve(view, base)
        if ctx == profile.DAMAGE_COUNTER:
            base = profile._damage_target(view, int(profile._HANDWRITTEN_STATE.get("adrena_count", 3)))
            selected = self._engine_threat_override(view, base)
            return self.target_protocol.resolve(view, selected)
        if ctx == profile.DAMAGE:
            base = profile._damage_target(view, 3, bench_only=True)
            selected = self._engine_threat_override(view, base)
            return self.target_protocol.resolve(view, selected)
        if ctx == profile.ATTACH_TO:
            return self.count_protocol.resolve_punkup(view, profile._punkup_count(view))
