...
from .matchup_router import ALAKAZAM, LUCARIO, MatchupRouter
from .strategic_memory import StrategicMemory
from .validated_fallback_policy import choose as choose_fallback
...
def agent(obs_dict: dict) -> list[int]:
    global _HISTORY, _STRATEGIC_MEMORY, _MATCHUP_ROUTER
    select = obs_dict.get("select")
    if select is None:
        _HISTORY = []
        _STRATEGIC_MEMORY.reset()
        _MATCHUP_ROUTER.reset()
    ...
    if state.get("turn") == 0 and state.get("context") == 41:
        _STRATEGIC_MEMORY.reset()
    matchup_mode = _MATCHUP_ROUTER.observe(obs_dict)
    action = apply_manual_guards(obs_dict)
    if action is None:
        if matchup_mode in (ALAKAZAM, LUCARIO):
            action = choose_fallback(state, options, _HISTORY)
        else:
