...
状态：**已提交、等待结算（Kaggle ref `55499962`，2026-08-14 14:49 CST）**。
8/15 的 challenger 定版为 Grim 固定牌组 + v22 纯规则 pilot；v28 和 v29 不再晋级。
...
5. 不同提交的早期/未结算 public score（包括 700.3、859.1 等）只记为背景，
   不当作同一机器人随时间的轨迹，也不当作配对 H2H 证据；候选排序只用按
   `submission_id` 归属的 episodes 与本地隔离闸。

## 线上 incumbent 的诚实读法

线上 public score 不是“一个机器人继续打”的时间序列：

- `55468450`（retreat pivot）于 8/13 06:54 CST 提交，曾显示 700.3；
- `55451759`（config A resubmit）于 8/12 15:20 CST 提交，曾显示 607.3；
- `55496363` 是 8/14 11:03 CST 的 retreat 精确恢复件，14:47 只读时为 645.2。

三者是不同 submission instance、不同 matchmaking/episode pool，不能拿 700.3、
607.3、645.2 互相作“同 bot 回落/提升”的因果比较。可比的弱证据是按 ref 拉取的
episodes：`55496363` 截至 13:48 的快照为 28 局 15-13（53.6%），与同代码旧探针
合并为 76 局 42-34（55.3%，Wilson 95% 约 44.1%-65.9%）；config A 参考为
42 局 20-22（47.6%）。这只支持“retreat 方向偏正、尚未显著”，不支持 public
score 的直接比较。

## 发射与收官

- 8/14 14:49 已提交上述归档（ref `55499962`），初始状态 `PENDING`；今晚只读
  轮询状态/错误，不把早期分数当结算真值。
- 8/15 确认日配额重置后尽早（目标 08:05 CST）按 tie-break 选择 v22 或 retreat
  的精确归档再提交；20:00 做首轮 live 盘点，不再等到晚上才发射。
