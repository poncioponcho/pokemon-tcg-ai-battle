# Pokemon TCG AI Battle Challenge —— v23.2 Mega Lucario ex Agent

参加 Kaggle [Pokemon TCG AI Battle Challenge](https://www.kaggle.com/competitions/pokemon-tcg-ai-battle)
的提交方案。当前主线为 **v23.2 メガルカリオex 能量循环墙推** 单文件 Agent，
在 v23.1 的 600.0 评分版本上加入 1 张含羞苞节奏锁变体。
...
| 宝可梦 | 235 含羞苞 (草/30HP) | 1 | 痒痒花粉: 0 能量 10 伤害，封锁对手下回合 Item |
| 训练家 | ゴング/リーリエ/ポケパッド/ハイパーボール 等 | 28 | 检索/抽牌/治疗/干扰/增伤 |
| 能量 | 闘×13 + ロック闘×4 | 17 | 斗系能量体系 |

**决策链 (18 步 + 前置耗牌):** 盾牌耗牌(打不动时磨光对手牌库) → KO 攻击(含弱点×2, 节能选最低伤害) → 铺场 → 进化 678 成核心 → 进化 674 破墙 → 主动含羞苞优先痒痒花粉 → 678 满能斩杀 → 冲刺模式 → 训练家时机 → 进化 → 贴能(辅助满能时能量改道给 678) → 攻击 → 出训练家/宝可梦 → bench 贴能 → 特性(ルナサイクル) → 兜底攻击 → 切换(678↔辅助, 奖赏节奏) → END。
...
```
.
├── main.py           # 提交入口: v23.2 自包含 Agent (内联牌组 + 全量卡库 1267 卡)
...
7. **稳定性验证**: 148,772 次真实对局决策 0 崩溃 0 非法动作; 本地功能测试 24/24,
   真实回放回归 945 决策 0 异常 0 非法动作, 压力测试 37,556 次决策全部通过。
...
| 版本 | 评分 | 说明 |
|------|------|------|
| v23.2 | 600.0 | 当前: 含羞苞节奏锁 (235 替换 1252) + 痒痒花粉优先级 + 审计修复 |
| v23.1 | 600.0 | P1 策略调优(节能KO+能量改道) + 3 bug 修复 |
