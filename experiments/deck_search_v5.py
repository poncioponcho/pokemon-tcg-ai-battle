# -*- coding: utf-8 -*-
"""deck_search_v5.py — v4 FINAL 基线深化爬山 (08-10 午后, v24.8 主体)。

与 deck_search_v4 的差异:
1. baseline 自动 = 现役 deck.csv (v4 FINAL: -674+1152, 55390992→v4 已提交
   12:12)。若 v4 LB 结算翻车, v5 结果作废、改从 55390992 牌组
   (deck.csv.bak-20260810-prev4) 重起 — 明日结算后拍板 (peer 12:33 边界)。
2. LOG/CKPT 独立命名空间 deck_search_v5*.jsonl。
3. 机制/候选面/联合适应度完全继承 v4 (agent=现役 main.py v24.8;
   CANDIDATES 18 张含 1235; 1158/1252 排除)。

⚠️ v5 只产候选, 绝不自动提交 (今日额度已用完; 提交前提 = 用户看过
v4 结算并显式确认)。搜完后走 3 种子标准闸 (seed 9000/9001/9002,
mirror≥0.7465 或 vs_first≥0.4564 且 mirror≥0.7095,
硬闸=卡池快败+ci_lo>0.5+invalid=0)。

用法：nice -n 10 python3 experiments/deck_search_v5.py [--rounds 3] [--screen-n 1000] [--confirm-n 4000] [--workers 3]
"""
...
LOG = RUNS / 'deck_search_v5.jsonl'
CKPT = RUNS / 'deck_search_v5_ckpt.jsonl'
...
def main():
    ...
    emit({'ev': 'start', 'ts': time.time(),
          'msg': f'=== deck_search_v5 start: rounds={args.rounds} screen_n={args.screen_n} '
