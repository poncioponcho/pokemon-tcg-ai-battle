# -*- coding: utf-8 -*-
"""deck_search.py — 牌组构筑贪心爬山（单卡置换坐标下降）。

依据：deck_policy_isolation 实测卡组效应 +0.111（vs first），
但 OUR vs SAMPLE rules 镜像 n=8000 wr=0.5439（OUR 直接更强）。
→ 以 OUR_DECK 为基底做贪心单卡置换，目标函数 = 先后手平衡镜像胜率。

流程（每轮）：
  1. 枚举合法单卡置换（移除 base 中 1 张 a，加入候选池 1 张 b）：
     - 非能量卡 ≤4 张；能量不限；牌组恒 60 张
  2. 筛选：rules(var) vs rules(base) 平衡镜像 n=SCREEN_N（seed0=1000）
  3. 确认：top-K 用独立种子集（seed0=5000）n=CONFIRM_N 重测，
     Wilson CI 下界 > 0.5 才接受（防筛选种子过拟合）
  4. 接受最优置换 → 更新 base，进入下一轮；无改进则停止
  5. 附记：每个被接受的牌组对 first(SAMPLE) 的胜率（LB 相关对局）

产物：experiments/runs/deck_search.jsonl（每行一个事件），stdout 同步打印。
用法：python3 experiments/deck_search.py [--rounds 3] [--screen-n 1000] [--confirm-n 8000]
"""
# [v2 可复现加固] PYTHONHASHSEED 须在解释器启动前固定, 进程内赋值无效 → re-exec 守卫
import os as _os
import sys as _sys
...
LOG = RUNS / 'deck_search.jsonl'
CKPT = RUNS / 'deck_search_ckpt.jsonl'

OUR_DECK = [int(l.strip()) for l in (PROJ / 'deck.csv').read_text().splitlines() if l.strip()]
SAMPLE = ar.SAMPLE_DECK

# 候选池：两套已知强牌组的并集（均为经实战/镜像验证的卡）
CANDIDATES = sorted(set(OUR_DECK) | set(SAMPLE))

# 卡 DB（判定能量卡，来自 champion main.py 内嵌库）
...
# ---- 并行 worker：每个进程独立加载一次 rules agent，置换间完全独立 ----
# seed0 仅固定 Python 侧随机；native shuffle 未注种，每个置换
# 仍是独立批次，不具有 paired/CRN 语义。
_W = {}


def _init_worker():
    _W['rules'] = ar.load_opponent('v23_2_rules')
...
def _eval_swap(task):
    ...
    try:
        ...
        vf = None
        if do_vf:
            # [v2 联合适应度] 仅 confirm 任务付 vs_first 腿代价 (screen 保持只跑 mirror)
...
def main():
    ...
    emit({'ev': 'start', 'ts': time.time(),
          'msg': f'=== deck_search start: rounds={args.rounds} screen_n={args.screen_n} '
                 f'confirm_n={args.confirm_n} candidates={len(CANDIDATES)} ==='})

    # 基线参考：base vs first(SAMPLE) — [v2] 同时作为 vs_first 主目标的当前值
    ...
    for rd in range(1, args.rounds + 1):
        ...
        # [v2 联合接受准则] 主目标 vs_first 最大; 硬约束 mirror 不显著劣于 base (wr>=0.49)
        best = None
        for w0, lo0, a, b in top:
            wr, lo, vf = cmap[(a, b)]
