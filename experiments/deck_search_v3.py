# -*- coding: utf-8 -*-
"""deck_search_v3.py — 聚焦版牌组爬山：只搜 bench/检索类单卡置换 (候选B, 08-09 夜场)。

与 deck_search.py 的差异仅三处: LOG/CKPT 独立命名空间 (deck_search_v3*.jsonl)、
CANDIDATES 聚焦 add 侧 (bench 宝可梦 + 检索/抽滤/干扰训练家, 见下)。
机制(筛选→独立种子确认→vs_first 主目标/mirror≥0.49 约束→断点续跑)完全继承。

产物：experiments/runs/deck_search_v3.jsonl（每行一个事件），stdout 同步打印。
用法：nice -n 10 python3 experiments/deck_search_v3.py [--rounds 3] [--screen-n 1000] [--confirm-n 4000] [--workers 3]
"""
...
LOG = RUNS / 'deck_search_v3.jsonl'
CKPT = RUNS / 'deck_search_v3_ckpt.jsonl'

OUR_DECK = [int(l.strip()) for l in (PROJ / 'deck.csv').read_text().splitlines() if l.strip()]
SAMPLE = ar.SAMPLE_DECK

# [v3 聚焦] 只搜 bench/检索类单卡置换 (peer 2026-08-09 23:00 指令, 候选B):
# add 侧 = bench 宝可梦(674/675/676) + 检索/抽滤/干扰训练家
# (1097/1102/1121/1142/1145/1152/1205/1213/1227);
# 排除能量/打手换位/1158/1235(卡池 gap, 未移植)。remove 侧保持全开。
