"""merge_gate_retreatpivot.py — retreat_pivot 候选三配置同批闸 (全 9 腿加权协议)

候选: submission_baseline/main.py FLAG_RETREAT_PIVOT
  防守性 prize-denial pivot: 3奖身 active 进对手下回合KO射程 + bench有<3奖替死鬼
  + post-KO锚 + 无KO计划 → RETREAT 2500 (压过 ATTACK, 低于关键PLAY)。
  证据: top_pilot diff 付费 retreat 我们0.42/局 vs Sixth 2.14/Dipam 1.76/ミワ 1.50。
  本地闸半可见: prize-denial 对 first-pilot 也成立 (傻瓜也打暴露3奖身) → 闸过即真候选。

三配置 (精确隔离 pivot 效应, dying_674/nrg_bench 均 NO-GO 排除):
  pristine : runs/_configA 快照 (config A 提交件)
  OFF      : FLAG_DYING_674=False, FLAG_NRG_BENCH=False, FLAG_RETREAT_PIVOT=False (parity 机械)
  ON       : 仅 FLAG_RETREAT_PIVOT=True (候选)
9 腿 × n=2000, 引擎不可播种 → 同一 pool batch 内交错测量, 无配对二项抽样 (#111)。

判决 (advisor 2026-08-12 20:05):
  parity    : |OFF − pristine| 每腿 & 加权 < 2.2pp
  regression: 任一腿 ON−pristine < −2.2pp 即不过 (重点盯 Alakazam/Crustle/other)
  提交判据  : 全 9 腿加权 ON−pristine > +2.2pp 且以上全过且 invalid ON==0 → 才谈提交
  保真度    : 半可见闸, 闸过即有意义; 交后仍按 #132 管线重测 live WR

用法: /opt/homebrew/bin/python3 experiments/merge_gate_retreatpivot.py [--n 2000] [--workers 3]
产物: experiments/runs/merge_gate_retreatpivot.json
"""
import argparse
import json
...
OUT = PROJ / 'experiments/runs/merge_gate_retreatpivot.json'
...
def _load_merged(dying: bool, nrg: bool, pivot: bool):
    m = ar.load_module(MERGED)
    m.FLAG_DYING_674 = dying
    m.FLAG_NRG_BENCH = nrg
    m.FLAG_RETREAT_PIVOT = pivot
    return m
...
def _init_worker():
    G['mods'] = {
        ...
        'OFF': _load_merged(False, False, False),
        'ON': _load_merged(False, False, True),
    }
...
def main():
    ...
    # 防御: 盘上 main.py 的 flag 默认值不影响闸 (worker 显式覆写), 但候选编辑必须已落盘
    _src = MERGED.read_text()
    assert 'FLAG_RETREAT_PIVOT' in _src and '_pivot_save_active' in _src, 'retreat_pivot 编辑未落盘!'
    ...
    verdict = ('GO: retreat_pivot > config A +2.2pp, 贴 advisor 定交不交' if go
