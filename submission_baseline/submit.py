...
def _validated_https_url(value: str) -> str:
    ...
    if (
        ...
        raise ValueError("Kaggle upload endpoint is not a valid HTTPS URL")
    return value
    ...
    try:
        with tarfile.open(TAR_PATH, "r:gz") as t:
            ...
            if extracted is None:
                return None
            return hashlib.sha256(extracted.read()).hexdigest()[:16]
    except Exception:
        return None


def _auto_desc(live_sha: str) -> str:
    """[2026-08-09 修复] 提交描述不再硬编码版本号（旧版固定 "v23.1"，导致
    v23.2 提交也被标成 v23.1，ref 55347848 血缘错乱）。改为从已校验的
    提交物自动派生：rules/hybrid 标记 + main.py sha + deck.csv 指纹。"""
    kind = "rules"
    try:
        with tarfile.open(TAR_PATH, "r:gz") as t:
            if "model_student.npz" in t.getnames():
                kind = "hybrid"
    except Exception:
        pass
    try:
        with open(os.path.join(WORK_DIR, "deck.csv"), "rb") as f:
            deck_fp = hashlib.sha256(f.read()).hexdigest()[:8]
    except OSError:
        deck_fp = "unknown"
    return (f"PTCG {kind} main:{live_sha[:12]} deck:{deck_fp} "
...
def cmd_submit():
    ...
    create_url = _validated_https_url(upload_resp.create_url)
    ...
    req = urllib.request.Request(  # noqa: S310 - URL validated above
        create_url, data=file_data, method="PUT")
    ...
    submit_req.submission_description = _auto_desc(live_sha)
    print(f"  提交描述: {submit_req.submission_description}")
